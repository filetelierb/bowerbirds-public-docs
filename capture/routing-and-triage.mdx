...
Classification on the shared Triage shelf is a **router** supervising the shelf Bucket: a direct router with one route per destination, tried in the order you arrange them, or an agentic router whose routes Bowerbirds AI delegates to. What the router gathers off the shelf at all is its own scope — the folders it watches there, and its global kind, keyword and age filters. Either way the router only adds — it derives a copy into the destination and signs the source with a status, and a cleanup rule on the shelf retires `routed` sources after a grace period — so a record is never silently moved. What used to be the `suggest` mode is the router's attention queue: a route that is not sure asks, with alternatives and a recommendation. See [Routers](/cloud/routers).
...
A Web Capture intake is a separate source with a router of its own — or, since a router may supervise several Buckets, a Bucket of the shelf's router alongside the shelf; see [Web Capture](/cloud/web-capture#routing-submitted-reports).
