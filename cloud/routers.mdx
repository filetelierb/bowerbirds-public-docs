...
- **Rules** are prose plus filter chips the gate checks deterministically: `kind` (`is`, `isNot`, `in`, `notIn`), `path` (`startsWith`), `tag` and `keyword` (`has`, `hasNot`) and `ageDays` (`atMost`). A direct route with no chips is the catch-all row. Rules only narrow which records a route may take; what a route is *for* is its description and goal, and that is what the orchestrator reads a record's findings against.
