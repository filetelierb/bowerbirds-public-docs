...
<CardGroup cols={2}>
  ...
  <Card title="Routers only add" icon="plus">
    A route never moves, edits or trashes a source record. It derives new records inside the organization, or creates, appends to or links items at external destinations. Deleting is a cleanup policy's job.
  </Card>
...
- **Rules** are prose plus filter chips the gate checks deterministically: `kind` (`is`, `isNot`, `in`, `notIn`), `path` (`startsWith`), `tag` and `keyword` (`has`, `hasNot`), `ageDays` (`atMost`), and over the record's [media context](/cloud/media#the-rule-chips), `context` (`has`, `hasNot`) and `signal` (`atLeast`). A direct route with no chips is the catch-all row.
- **Media** is the router's option for the [media processor](/cloud/media#the-routers-media-option): `"media": { "process": "auto", "videoResolution": "low", "maxMinutesPerRecord": 60 }` reads every gathered record's recordings, audio, PDFs and pictures before the cycle plans, so a route decides from a digest and graded signals rather than a title and a few stills.
- **Skills** are organization skills by id, snapshotted into the run. A plugin can ship a route template with all of this plus the connector it expects; see [Plugins](/integrations/plugins).
...
graph LR
  ...
  M --> T[Triage: rule table, or Bowerbirds AI delegates each record to the routes that fit]
...
3. **Media.** Each gathered record with media and no current context is read by the [media processor](/cloud/media): its digest, signals, entities and screens are what triage sees. A record already read at its current version costs nothing again.
4. **Triage.** A direct router evaluates its rule table. An agentic router reads each record's digest and signals, delegates it to every route whose description fits with a hint of which part is theirs, and hands each route its whole batch, so three captures about one bug are seen together.
5. **Route.** Each route searches its destinations with its enabled read tools, reads deeper into a record's context with `get_record_context` when the digest is not enough, groups what belongs together, and answers per record.
...
8. **Report.** Exceptions become attention rows; the ledger gets one line per model turn with the job id; the run record holds the dispatches, the gate's decisions, the media stage's counts and the cost.
...
| `pending` | A route still owes an answer, or the organization's routers are paused |
...
- The daily sweep **trashes** matching sources; nothing is deleted for good until the organization's own retention purges the Trash.
...
- `pending` is refused as a rule status and never touched by any rule, so an organization that ran out of credits cannot lose the records it froze.
...
- The **budget pre-flight** compares the organization balance and the router's `budget.monthlyCredits` against the gathered records before spending anything. Manual runs honour the same caps.
...
- The **media stage** is priced in the pre-flight from a probe of each record's media. When plan plus media would exceed the balance or the router's month, media is switched off for that cycle with a note, never a pause; see [Media context](/cloud/media#credits).
- Out of credits or out of storage **pauses every router of the organization** (`routersPaused {reason: credits | storage, since}` on the organization). The sources the paused cycle gathered stay `pending` and render as frozen in both apps; an inbox notice says what to buy or free. A top-up or freed space enqueues the resume by itself, and the resumed cycle re-delegates only the routes still missing, so no dispatch is duplicated.
